...
// Deep-clone: default behavior creates a generic AstNode with same NodeType,
// copies line/col and deep-clones children. TerminalNode overrides this to
// preserve token data.
AstNode* AstNode::clone() const {
    AstNode* n = new AstNode(this->type);
    n->line = this->line;
    n->col = this->col;
    for (const auto* c : this->children) {
        if (c) n->add_child(c->clone());
    }
    return n;
}
...
AstNode* TerminalNode::clone() const {
    TerminalNode* t = new TerminalNode(this->token_type, this->value, this->name, this->line, this->col);
    for (const auto* c : this->children) {
        if (c) t->add_child(c->clone());
    }
    return t;
}
